...
static void fix_ehdr_byte_order( elf_file_handle elf_file_hnd, struct Elf32_Ehdr *e_hdr )
{
    /* note that one of the branches will always get compiled out, depending on host endianness */
...
static void fix_ehdr64_byte_order( elf_file_handle elf_file_hnd, struct Elf64_Ehdr *e_hdr )
{
    /* note that one of the branches will always get compiled out, depending on host endianness */
    if( elf_file_hnd->flags & ORL_FILE_FLAG_BIG_ENDIAN ) {
        CONV_BE_16( e_hdr->e_type );
...
static void fix_shdr_byte_order( elf_file_handle elf_file_hnd, struct Elf32_Shdr *e_shdr )
{
    /* note that one of the branches will always get compiled out, depending on host endianness */
    if( elf_file_hnd->flags & ORL_FILE_FLAG_BIG_ENDIAN ) {
...
static void fix_shdr64_byte_order( elf_file_handle elf_file_hnd, struct Elf64_Shdr *e_shdr )
{
    /* note that one of the branches will always get compiled out, depending on host endianness */
    if( elf_file_hnd->flags & ORL_FILE_FLAG_BIG_ENDIAN ) {
        CONV_BE_32( e_shdr->sh_name );
 ...
 * how the rest of the data is to be interpreted
  */
static void determine_file_class( elf_file_handle elf_file_hnd, struct Elf32_Ehdr *e_hdr )
{
...
static void determine_section_specs( elf_sec_handle elf_sec_hnd, elf_word sh_type, uint32_t sh_flags )
{
...
{
    struct Elf32_Shdr *s_hdr32;
    struct Elf64_Shdr *s_hdr64;
    ...
    for( i = 0; i < elf_file_hnd->num_sections; ++i ) {
        elf_sec_hnd = (elf_sec_handle)_ClientAlloc( elf_file_hnd, sizeof( struct elf_sec_handle_struct ) );
        ...
        if( elf_file_hnd->flags & ORL_FILE_FLAG_64BIT_MACHINE ) {
            s_hdr64 = (struct Elf64_Shdr *)s_hdr;
            ...
            elf_sec_hnd->file_offset = s_hdr64->sh_offset;
            ...
            s_hdr32 = (struct Elf32_Shdr *)s_hdr;
        ...
        /* TODO: What if the alignment was degenerate? */
...
{
    char *s_hdr;
    struct Elf32_Ehdr *e_hdr32;
    struct Elf64_Ehdr *e_hdr64;
    elf_quantity contents_size1, sec_header_table_size;
    elf_index *name_index;
    elf_file_offset shoff;
    elf_half shnum, ehsize, shstrndx;
    ...
    if( elf_file_hnd->flags & ORL_FILE_FLAG_64BIT_MACHINE ) {
        e_hdr64 = _ClientRead( elf_file_hnd, sizeof( struct Elf64_Ehdr ) );
        if( e_hdr64 == NULL ){
            return ORL_ERROR;
        }
        ...
        e_hdr32 = _ClientRead( elf_file_hnd, sizeof( struct Elf32_Ehdr ) );
        if( e_hdr32 == NULL ){
            return ORL_ERROR;
        }
    ...
    contents_size1 = shoff - (elf_file_offset)ehsize;
    ...
    {
        const orl_return return_val = load_elf_sec_handles( elf_file_hnd, name_index, s_hdr );
    ...
    // TODO: This is probably not necessary now.
    ...
    {
        elf_quantity i;
        char *const string_table = (char *)elf_file_hnd->orig_sec_handles[shstrndx - 1]->contents;
        for( i = 0; i < elf_file_hnd->num_sections; i++ ) {
