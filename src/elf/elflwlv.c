...
static void fix_sym_byte_order( elf_file_handle elf_file_hnd, struct Elf32_Sym *e_sym )
{
...
static void fix_sym64_byte_order( elf_file_handle elf_file_hnd, struct Elf64_Sym *e_sym )
{
    // note that one of the branches will always get compiled out,
    // depending on host endianness
    if( elf_file_hnd->flags & ORL_FILE_FLAG_BIG_ENDIAN ) {
...
static void fix_rel_byte_order( elf_file_handle elf_file_hnd, struct Elf32_Rel *e_rel )
{
    // note that one of the branches will always get compiled out,
    // depending on host endianness
    if( elf_file_hnd->flags & ORL_FILE_FLAG_BIG_ENDIAN ) {
...
static void fix_rela_byte_order( elf_file_handle elf_file_hnd, struct Elf32_Rela *e_rela )
{
    // note that one of the branches will always get compiled out,
    // depending on host endianness
    if( elf_file_hnd->flags & ORL_FILE_FLAG_BIG_ENDIAN ) {
...
static void fix_rel64_byte_order( elf_file_handle elf_file_hnd, struct Elf64_Rel *e_rel )
{
    // note that one of the branches will always get compiled out,
    // depending on host endianness
    if( elf_file_hnd->flags & ORL_FILE_FLAG_BIG_ENDIAN ) {
...
static void fix_rela64_byte_order( elf_file_handle elf_file_hnd, struct Elf64_Rela *e_rela )
{
    // note that one of the branches will always get compiled out,
    // depending on host endianness
    if( elf_file_hnd->flags & ORL_FILE_FLAG_BIG_ENDIAN ) {
...
orl_return ElfCreateSymbolHandles( elf_sec_handle elf_sec_hnd )
{
    ...
    struct Elf32_Sym *current_sym32;
    struct Elf64_Sym *current_sym64;
    uint32_t st_name;

    const uint64_t num_syms = elf_sec_hnd->size / elf_sec_hnd->entsize;
    elf_sec_hnd->assoc.sym.symbols = (elf_symbol_handle)_ClientSecAlloc( elf_sec_hnd, sizeof( struct elf_symbol_handle_struct ) * num_syms );
    ...
    for( i = 0; i < num_syms; i++ ) {
        if( elf_sec_hnd->elf_file_hnd->flags & ORL_FILE_FLAG_64BIT_MACHINE ) {
            current_sym64 = (struct Elf64_Sym *)current_sym;
            ...
            current_sym32 = (struct Elf32_Sym *)current_sym;
...
orl_return ElfBuildSecNameHashTable( elf_file_handle elf_file_hnd )
{
    ...
    for( i = 0; i < elf_file_hnd->num_sections; ++i ) {
        ...
        const orl_return return_val = ORLHashTableInsert( elf_file_hnd->sec_name_hash_table, h_key, h_data );
...
orl_return ElfCreateRelocs( elf_sec_handle orig_sec, elf_sec_handle reloc_sec )
{
    if( reloc_sec->assoc.reloc.symbol_table->assoc.sym.symbols == NULL ) {
        const orl_return return_val = ElfCreateSymbolHandles( reloc_sec->assoc.reloc.symbol_table );
        if( return_val != ORL_OKAY ) {
            return return_val;
        }
    }
        ...
        uint64_t i;
        const uint64_t num_relocs = reloc_sec->size / reloc_sec->entsize;
        unsigned char *irel = reloc_sec->contents;
        struct orl_reloc_struct *orel = _ClientSecAlloc( reloc_sec, sizeof( struct orl_reloc_struct ) * num_relocs );
        ...
        if(reloc_sec->type == ORL_SEC_TYPE_RELOCS){
            for( i = 0; i < num_relocs; i++ ) {
                ...
                if( reloc_sec->elf_file_hnd->flags & ORL_FILE_FLAG_64BIT_MACHINE ) {
                    struct Elf64_Rel *const irel64 = (struct Elf64_Rel *)irel;
                    fix_rel64_byte_order( reloc_sec->elf_file_hnd, irel64 );
                    /* TODO: Why do we use the 32 high bits? */
                    orel->symbol = (orl_symbol_handle)( reloc_sec->assoc.reloc.symbol_table->assoc.sym.symbols + (irel64->r_info >> 32) );
                    orel->type = ElfConvertRelocType( reloc_sec->elf_file_hnd, (elf_reloc_type)irel64->r_info );
                    orel->offset = irel64->r_offset;
                }
                else {
                    struct Elf32_Rel *const irel32 = (struct Elf32_Rel *)irel;
        ...
        else if(reloc_sec->type == ORL_SEC_TYPE_RELOCS_EXPADD){
            for( i = 0; i < num_relocs; i++ ) {
                orel->section = (orl_sec_handle)orig_sec;
                orel->frame = NULL;
                if( reloc_sec->elf_file_hnd->flags & ORL_FILE_FLAG_64BIT_MACHINE ) {
                    struct Elf64_Rela *const irela64 = (struct Elf64_Rela *)irel;
                    fix_rela64_byte_order( reloc_sec->elf_file_hnd, irela64 );
                    /* TODO: Why do we use the 32 high bits? */
                    orel->symbol = (orl_symbol_handle)( reloc_sec->assoc.reloc.symbol_table->assoc.sym.symbols + (irela64->r_info >> 32) );
                    orel->type = ElfConvertRelocType( reloc_sec->elf_file_hnd, (elf_reloc_type)irela64->r_info );
                    orel->offset = irela64->r_offset;
                    orel->addend = irela64->r_addend;
                }
                else {
                    struct Elf32_Rela *const irela32 = (struct Elf32_Rela *)irel;
...
orl_return ElfParseDrectve( const char *contents, size_t len, struct orl_note_callbacks *cbs, void *cookie )
/***************************************************************************************************/
{
    EatWhite( &contents, &len );
    while( len > 0 ) {
        const char *const cmd = contents+1;
        if( *contents != '-' )
