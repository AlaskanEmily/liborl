...
static bool determine_file_specs( coff_file_handle coff_file_hnd, struct coff_file_header *f_hdr )
/*****************************************************************************************/
{
    uint16_t cpu_type;
    uint16_t flags;
    ...
    if( ( f_hdr->cpu_type == IMAGE_FILE_MACHINE_UNKNOWN ) && ( f_hdr->num_sections == IMPORT_OBJECT_HDR_SIG2 ) ) {
        /* COFF import library */
        cpu_type = ((struct coff_import_object_header *)f_hdr)->machine;
        flags = 0;
...
static void determine_section_specs( coff_sec_handle coff_sec_hnd,
    struct coff_section_header *s_hdr )
/****************************************************************/
{
...
/**********************************************************************/
{
    ...
    if( coff_file_hnd->num_sections == 0 ) {
        ...
        reloc_sec_offset = _ClientAlloc( coff_file_hnd, sizeof( coff_sec_offset ) * coff_file_hnd->num_sections);
        reloc_sec_size = _ClientAlloc(coff_file_hnd, sizeof( coff_sec_size ) * coff_file_hnd->num_sections);
    ...
    s_hdr = (struct coff_section_header *)coff_file_hnd->s_hdr_table_buffer;
    for( i = 0; i < coff_file_hnd->num_sections; ++i ) {
        coff_sec_hnd = (coff_sec_handle)_ClientAlloc( coff_file_hnd, sizeof( struct coff_sec_handle_struct ) );
        ...
        reloc_sec_size[i] = s_hdr->num_relocs * sizeof( struct coff_reloc );
        s_hdr++;
    }
    /* There are num_reloc_secs + 2 additional section handles to create (one each for the symbol and string tables) */
    ...
    for( i = 0; i < coff_file_hnd->num_sections; ++i ) {
        const coff_sec_size size = reloc_sec_size[i];
        if( size > 0 ) {
            reloc_secs_created++;
            /* create a reloc section */
            coff_file_hnd->coff_sec_hnd[coff_file_hnd->num_sections + reloc_secs_created] = (coff_sec_handle)_ClientAlloc( coff_file_hnd, sizeof( struct coff_sec_handle_struct ) );
    ...
    coff_file_hnd->symbol_table = (coff_sec_handle)_ClientAlloc( coff_file_hnd, sizeof( struct coff_sec_handle_struct ) );
    ...
    coff_file_hnd->symbol_table->size = f_hdr->num_symbols * sizeof( struct coff_symbol );
    ...
    coff_file_hnd->string_table = (coff_sec_handle)_ClientAlloc( coff_file_hnd, sizeof( struct coff_sec_handle_struct ) );
...
/****************************************************************/
{
    ...
    coff_file_hnd->f_hdr_buffer = _ClientRead( coff_file_hnd, sizeof( struct coff_file_header ) );
    f_hdr = (struct coff_file_header *)coff_file_hnd->f_hdr_buffer;
    if( determine_file_specs( coff_file_hnd, f_hdr ) ) {
         ...
         */
        {
            const orl_return return_val = convert_import_library_init( coff_file_hnd );
    ...
    coff_file_hnd->initial_size = sizeof( struct coff_file_header ) + f_hdr->opt_hdr_size + PEoffset;
    ...
    sec_header_table_size = coff_file_hnd->num_sections * sizeof( struct coff_section_header );
    if( coff_file_hnd->num_sections > 0 ) {
    ...
    {
        const orl_return return_val = load_coff_sec_handles( coff_file_hnd, f_hdr );
        if( return_val != ORL_OKAY ) {
            return return_val;
        }
    }
