...
typedef uint8_t omf_file_flags;
typedef uint32_t omf_file_index;
typedef uint32_t omf_headers_size;

typedef uint32_t omf_sec_flags;
typedef uint32_t omf_sec_size;
typedef uint32_t omf_sec_offset;
typedef int32_t omf_sec_addend;
typedef uint32_t omf_sec_base;

typedef uint16_t omf_reloc_type;

typedef uint32_t omf_symbol_flags;
typedef uint32_t omf_symbol_value;

typedef uint16_t omf_idx;
typedef uint16_t omf_frame;

typedef int32_t omf_rec_size;
typedef uint8_t omf_string_len;

typedef int32_t omf_quantity;

typedef uint8_t *omf_bytes;
typedef uint8_t omf_rectyp;
typedef int8_t omf_dbg_style;

struct orl_omf_tmp_lidata_struct {
    omf_rec_size size;
    omf_rec_size used;
    uint32_t offset;
    bool is32;
...
struct orl_omf_tmp_fixup_struct {
    omf_tmp_fixup next;
    bool is32;
    int mode;
    int location;
...
struct orl_omf_tmp_bakpat_struct {
    omf_tmp_bkfix       first_fixup;
    omf_tmp_bkfix       last_fixup;
};

struct orl_omf_tmp_bkfix_struct {
    omf_tmp_bkfix       next;
...
struct orl_omf_thred_fixup_struct {
    omf_idx             idx;
    unsigned char       method;
};

struct orl_omf_file_handle_struct {
    omf_handle omf_hnd;
    ...
    struct orl_omf_thred_fixup_struct frame_thred[4];
    struct orl_omf_thred_fixup_struct target_thred[4];
...
struct omf_seg_assoc_struct {
    ...
    struct omf_comdat_struct comdat;
    omf_grp_handle group;
    orl_linnum lines;
...
struct orl_omf_sec_handle_struct {
...
struct orl_omf_symbol_handle_struct {
    orl_file_format file_format;
    omf_file_handle omf_file_hnd;
    orl_symbol_type typ;
    int rec_typ;
    ...
    struct orl_omf_string_struct name;
};

struct orl_omf_grp_handle_struct {
    orl_file_format file_format;
    omf_file_handle omf_file_hnd;
    omf_idx name;
    omf_idx id;
...
struct orl_omf_scan_tab_struct {
    omf_idx seg;
    omf_idx lname;
